...
| other exact `chunk-*.ts` Mermaid runtime matches                                 | `mermaid` / `mermaid-k5` same-ID internal subpaths                        | package identity follows the published same-ID `.mjs`, not the generic name |
