...
var Ae = {
  ...
  Ke = new Set([`Z_BUF_ERROR`, `Z_DATA_ERROR`, `ERR_INVALID_STATE`]),
...
function U(e, t, n) {
  if (!Number.isFinite(e) || e < 0 || e > t)
    throw new H(`${n} has invalid size ${e} (maximum ${t} bytes)`);
  return e;
}
async function W(e, t, { maximumLength: n = M, reason: r = `skip` } = {}) {
  let i = U(t, n, r);
  await e.ignore(i);
}
async function G(
  ...
  let a = U(n?.length ?? t.length, r, i);
  return e.readBuffer(t, { ...n, length: a });
}
async function Je(e, { maximumLength: t = A } = {}) {
...
var K = 134695760,
  ...
  Ye = q - 1;
function Xe(e, t) {
  if (e.length < q) return -1;
  let n = e.length - q;
  for (let r = 0; r <= n; r++)
    if (m.get(e, r) === K && m.get(e, r + 8) === t + r) return r;
  return -1;
}
function Ze(e) {
...
async function $e(e, { shouldBuffer: t, maximumLength: n = A } = {}) {
  let { syncBuffer: r } = e,
    { length: i } = r,
    a = [],
    o = 0;
  for (;;) {
    let s = await e.tokenizer.peekBuffer(r, { mayBeLess: !0 }),
      c = Xe(r.subarray(0, s), o),
      l = c >= 0 ? 0 : s === i ? Math.min(Ye, s - 1) : 0,
      u = c >= 0 ? c : s - l;
    if (u === 0) break;
    if (((o += u), o > n))
  ...
  if ((X(e.tokenizer) || (e.knownSizeDescriptorScannedBytes += o), t))
    return Qe(a, o);
}
function et(e, t) {
  return X(e.tokenizer)
...
async function tt(
  ...
  if (t.dataDescriptor && t.compressedSize === 0)
    return $e(e, { shouldBuffer: n, maximumLength: r });
  if (!n) {
    await W(e.tokenizer, t.compressedSize, {
      maximumLength: X(e.tokenizer) ? A : e.tokenizer.fileInfo.size,
      reason: `ZIP entry compressed data`,
    });
    return;
  }
  let i = ct(e.tokenizer);
  if (
...
((D.prototype.inflate = async function (e, t, n) {
  ...
  return n(await Je(t, { maximumLength: A }));
}),
  (D.prototype.unzip = async function (e) {
    ...
    do {
      if (Z(this.tokenizer, r, M))
        throw new H(`ZIP stream probing exceeds ${M} bytes`);
      ...
      let o = await tt(this, i, {
        shouldBuffer: !!a.handler,
        maximumDescriptorLength: Math.min(A, et(this, r)),
      });
      if (
        (a.handler && (await this.inflate(i, o, a.handler)), i.dataDescriptor)
      ) {
        let e = new Uint8Array(q);
        if ((await this.tokenizer.readBuffer(e), m.get(e, 0) !== K))
          throw Error(
...
function lt(e) {
  if (e instanceof _ || e instanceof H) return !0;
  if (!(e instanceof Error)) return !1;
  if (We.has(e.message) || Ke.has(e.code)) return !0;
  for (let t of Ge) if (e.message.startsWith(t)) return !0;
  return !1;
}
function Q(e, t = A) {
...
var pt = class e {
  constructor(e) {
    let t = ot(e?.mpegOffsetTolerance);
  ...
  createTokenizerFromWebStream(e) {
    return qe(_e(e, this.getTokenizerOptions()));
  }
  async parseTokenizer(e, t = 0) {
    ...
    for (let t of this.detectors) {
      ...
      try {
        ...
        if (e instanceof _ || e instanceof H) return;
        throw e;
      }
      if (r) return r;
  ...
  async toDetectionStream(e, t) {
    let n = it(t?.sampleSize ?? 4100),
      r,
      i,
      a = e.getReader({ mode: `byob` });
    try {
      let { value: e, done: t } = await at(
        a,
        new Uint8Array(n),
        this.options.signal,
      );
  ...
  detectConfident = async (e) => {
    ...
    if (this.checkString(`ID3`)) {
      await W(e, 6, { maximumLength: 6, reason: `ID3 header prefix` });
          ...
          switch (e.filename) {
            ...
            case `mimetype`:
              return Q(e, P)
                ? {
                    async handler(e) {
                      t = J(new TextDecoder(`utf-8`).decode(e).trim());
                    },
                    stop: !0,
                  }
                : {};
    ...
    if (this.checkString(`OggS`)) {
      ...
      return (
        ...
        Y(t, [79, 112, 117, 115, 72, 101, 97, 100])
          ? { ext: `opus`, mime: `audio/ogg; codecs=opus` }
          : Y(t, [128, 116, 104, 101, 111, 114, 97])
            ? { ext: `ogv`, mime: `video/ogg` }
            : Y(t, [1, 118, 105, 100, 101, 111, 0])
              ? { ext: `ogm`, mime: `video/ogg` }
              : Y(t, [127, 70, 76, 65, 67])
                ? { ext: `oga`, mime: `audio/ogg` }
                : Y(t, [83, 112, 101, 101, 120, 32, 32])
                  ? { ext: `spx`, mime: `audio/ogg` }
                  : Y(t, [1, 118, 111, 114, 98, 105, 115])
    ...
    if (this.check([26, 69, 223, 163])) {
      ...
      async function r(t) {
        let r = 0;
        for (; t > 0;) {
          if ((r++, r > Le || Z(e, a, M))) return;
          let i = e.position,
            o = await n();
          if (o.id === 17026) {
            if (o.len > L) return;
            let t = U(o.len, L, `EBML DocType`);
            return (await e.readToken(new g(t))).replaceAll(/\00.*$/g, ``);
          }
          if (
            (X(e) && (!Number.isFinite(o.len) || o.len < 0 || o.len > R)) ||
            (await W(e, o.len, {
              maximumLength: X(e) ? R : e.fileInfo.size,
              reason: `EBML payload`,
            }),
            --t,
            e.position <= i)
      ...
      do {
        ...
        switch (l.type) {
          ...
          default:
            ...
            if (i && l.length > He) return s && Ze(l.type) ? t : void 0;
            try {
              await W(e, l.length + 4, {
                maximumLength: i ? 1048580 : e.fileInfo.size,
                reason: `PNG chunk payload`,
              });
            } catch (e) {
              if (!i && (e instanceof H || e instanceof _)) return t;
              throw e;
            }
        }
...
`)
    ...
    if (this.check([48, 38, 178, 117, 142, 102, 207, 17, 166, 217])) {
      ...
      try {
        ...
        await W(e, 30, { maximumLength: 30, reason: `ASF header prelude` });
        let r = X(e),
          i = e.position,
          a = 0;
        for (
          ;
          e.position + 24 < e.fileInfo.size && (a++, !(a > Be || Z(e, i, M)));
        ) {
          let i = e.position,
            a = await n(),
            o = a.size - 24;
          ...
          if (
            (await W(e, o, {
              maximumLength: r ? B : e.fileInfo.size,
        ...
        if (n instanceof _ || n instanceof H) X(e) && (t = !0);
        else throw n;
      }
...
`,
  ...
  async readTiffIFD(e) {
    let t = await this.tokenizer.readToken(e ? p : f);
    if (!(t > Ve) && !(X(this.tokenizer) && 2 + t * 12 > V))
      for (let n = 0; n < t; ++n) {
  ...
  async readTiffHeader(e) {
    ...
    if (n === 42) {
      ...
      if (X(this.tokenizer) && r > Ue) return t;
      let n = X(this.tokenizer) ? V : this.tokenizer.fileInfo.size;
      try {
        await W(this.tokenizer, r, {
          maximumLength: n,
...
(new Set(je), new Set(Me), (exports.fileTypeFromBuffer = rt));
//# sourceMappingURL=core-CTqnpHHs.js.map
