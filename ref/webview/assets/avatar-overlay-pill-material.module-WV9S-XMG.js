...
import { Or as ae } from "./app-initial~app-main~page-CtX5-cLy.js";
import {
  Gi as oe,
  Ji as se,
  Ki as _,
...
function ce(e, t, n = 1) {
  ...
  if (Math.hypot(r, i) <= C) return null;
  let a = (Math.atan2(r, -i) * (180 / Math.PI) + 360) % 360,
    o = Math.round(a / y) % b;
  return {
    columnIndex: o % S,
    frameDurationMs: 0,
    rowIndex: x + Math.floor(o / S),
  };
}
var y,
  b,
  ...
  C,
  le = e(() => {
    (ae(), (y = 22.5), (b = 16), (x = 9), (S = 8), (C = 1));
  });
function ue(e) {
  if (e == null) return T;
  if (e.kind === `first-awake`) return E;
  if (e.isLoading)
    return {
      ...
      fallbackBodyMessage: w.runningFallbackBody,
      iconClassName: `icon-xs shrink-0 text-token-text-secondary`,
      iconType: `spinner`,
      labelMessage: w.running,
      mascotState: `running`,
    };
  switch (e.level) {
    case `warning`:
  ...
  de = e(() => {
    (c(),
      ...
      (T = {
        badgeBackgroundColor: `var(--color-token-activity-bar-badge-background)`,
        badgeForegroundColor: `var(--color-token-activity-bar-badge-foreground)`,
        fallbackBodyMessage: w.info,
        iconClassName: `icon-xs shrink-0 text-token-text-secondary`,
        iconType: `clock`,
        labelMessage: w.info,
        mascotState: `idle`,
      }),
      (E = { ...T, mascotState: `waving` }));
  });
function fe(
  e,
  t,
...
function Se({
  ...
  includeMcpElicitationCancelAction: a,
  intl: o,
  excludedConversationId: s,
}) {
  if (t(e)) return null;
  let c = e.hostId ?? `local`,
    l = c === `local` ? `local` : `remote-host`,
    u = De(e),
    d = e.threadSource === `automation` || !1,
    f = d ? Te(e) : we(e, o),
    h = i && u === `waiting` ? p(e) : null;
  return {
    ...
    sortAtMs: n(e)?.turnStartedAtMs ?? e.updatedAt,
    status: u,
    subtitle: f,
    title: m(e) ?? o.formatMessage(G.newThread),
    turnKey: String(r(e)),
    updatedAtMs: e.updatedAt,
    waitingRequest: fe(h, o, {
      includeMcpElicitationCancelAction: a,
  ...
  let t = e.resumeState === `needs_resume` ? e.threadRuntimeStatus : null,
    r =
      e.resumeState === `needs_resume`
        ? t?.type === `active`
        : e.resumeState === `resuming` || n(e)?.status === `inProgress`,
    i =
      e.resumeState === `needs_resume`
...
//# sourceMappingURL=avatar-overlay-pill-material.module-WV9S-XMG.js.map
