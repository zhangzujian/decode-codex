...
function createPivotAreaReferenceValue() {
  return {
    value: void 0,
  };
}
export const PivotAreaReferenceValue = {
  encode(message, writer = new BinaryWriter()) {
    return (
      message.value !== void 0 && writer.uint32(8).uint32(message.value),
      writer
    );
  },
  decode(input, length) {
    let reader =
      ...
      message = createPivotAreaReferenceValue();
    for (; reader.pos < end;) {
  ...
  create(properties) {
    return PivotAreaReferenceValue.fromPartial(properties ?? {});
  },
  fromPartial(object) {
    let message = createPivotAreaReferenceValue();
